...
class Caching extends MusicBeatState
{
	...
	override function create()
	{
		...
		StenLogo = new FlxSprite(FlxG.width / 2, FlxG.height / 2).loadGraphic(Paths.loadImage('StenEngineLogo'));
		StenLogo.x -= StenLogo.width / 2;
		StenLogo.y -= StenLogo.height / 2 + 100;
		text.y -= StenLogo.height / 2 - 125;
		text.x -= 170;
		StenLogo.setGraphicSize(Std.int(StenLogo.width * 0.6));
		if (FlxG.save.data.antialiasing != null)
			StenLogo.antialiasing = FlxG.save.data.antialiasing;
		else
			StenLogo.antialiasing = true;
